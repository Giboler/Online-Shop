...
void Products::setProduct()
{
	...
	cout << "Enter the product Cost: "; cin >> productCost;
}
void Products::setID() {
	int counter = 0;
	fstream table;
...
void Products::unpack()
{
	...
	productCost = stoi(values[4]);
	productID = stoi(values[5]);
}
...
void Products::deleteProduct(string n)
{
	...
	table1.open("Products.txt", ios::in);
	while (!table1.eof())
	{
		getline(table1, line);
		if (line == "") {
...
}void Products::productStart()
{
	...
	char choice2;
	cout << "Would you like to do something else in the product section?(y/n): "; cin >> choice2;
	if (choice2 == 'y') {
		productStart();
	}
	else {
